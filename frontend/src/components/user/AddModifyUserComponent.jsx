...
const AddModifyUserComponent = async ({ userItem, currentUser, action, onConfirm }) => {
    ...
    if (currentUser.usertype === "SUPERADMIN") {
        types.push({ label: 'SuperAdministrador', value: "SUPERADMIN" });
    }

    // HTML para selects y opciones
    const optionsHtml = types
        .map(t => `<option value="${t.value}" ${userItem?.usertype === t.value ? "selected" : ""}>${t.label}</option>`)
        .join('');
    ...
    const swalStep = await Swal.fire({
        ...
        preConfirm: () => {
            ...
            let version = null;
            if (action === "modify" && userItem.version) { version = userItem.version; }

            return { username, password, usertype, version };
        }
    });
